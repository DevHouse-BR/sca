...
class PostagensController extends Zend_Controller_Action {
	...
	public function saveAction(){
		try{
			...
			$sca_clientes_id = (int)$this->getRequest()->getParam('sca_clientes_id');
			if($sca_clientes_id == 0){
				throw new Exception(DMG_Translate::_('controle.postagens.erro.cliente'));
			}
			
			$anexos = $this->getRequest()->getParam('anexos');
			$texto_postagem = $this->getRequest()->getParam('texto_postagem');
			$fl_publicado = (int)$this->getRequest()->getParam('fl_publicado');
			...
			$postagem->fl_publicado = $fl_publicado;
			...
			if(strlen($destinatarios)>0){
				...
				$email_template = str_replace ('{footer}', DMG_Config::getAccountCfg(7), $email_template);
				...
				$email_template = str_replace ('{nome_sistema}', DMG_Config::getAccountCfg(6), $email_template);
		...
		if($podeAcessar) {		
			...
			if (DMG_Acl::canAccess(23)) {
				...
				$query = "
						...
						";
				$query .= " WHERE p.sca_account_id = " . Zend_Auth::getInstance()->getIdentity()->sca_account_id . " ";
				$query .= " AND p.scm_user_id = " . Zend_Auth::getInstance()->getIdentity()->id . " ";
				...
				$query .= "
						...
						INNER JOIN scm_user u ON u.id = p.scm_user_id
						INNER JOIN sca_clientes c ON c.id = p.sca_clientes_id
						INNER JOIN sca_departamentos dp ON u.sca_departamentos_id = dp.id
						LEFT OUTER JOIN  (
							SELECT count(id) as leituras, sca_postagens_id FROM sca_logleituras GROUP BY sca_postagens_id
						) l ON l.sca_postagens_id = p.id
						LEFT OUTER JOIN  (
						...
						";
				$query .= " WHERE p.sca_account_id = " . Zend_Auth::getInstance()->getIdentity()->sca_account_id . " ";
				$query .= " AND dp.id_gerente = " . Zend_Auth::getInstance()->getIdentity()->id . " ";
				...
				$query .= "
						...
						";
				$query .= " WHERE p.sca_account_id = " . Zend_Auth::getInstance()->getIdentity()->sca_account_id . " ";
				$query .= " AND c.id_responsavel = " . Zend_Auth::getInstance()->getIdentity()->id . " ";
			}
	...
	public function publishAction(){
		try{
			...
			if($this->getRequest()->getParam('publish') == "true") $publish = true;
			else $publish = false;
	
			foreach($ids as $id){
				$query = Doctrine_Query::create()
					->update('ScaPostagens')
					->set('fl_publicado', '?', $publish)
					->where('id = ?', $id)
					->execute(); 
			}
	...
	public function listbyclientsAction(){
		$data = array();
		$podeAcessar = false;
		if (DMG_Acl::canAccess(21)) {
			$podeAcessar = true;
		...
		if($podeAcessar) {		
			...
			if(!DMG_Acl::canAccess(23))
				$query .= " AND p.scm_user_id IN (SELECT su.id FROM scm_user su WHERE su.sca_clientes_id IN (SELECT sc.id FROM sca_clientes sc WHERE sc.id_responsavel = ".Zend_Auth::getInstance()->getIdentity()->id.") )";
	...
	public function listminenreadAction(){
		$data = array();
		$podeAcessar = false;
		if (DMG_Acl::canAccess(21)) {
			$podeAcessar = true;
