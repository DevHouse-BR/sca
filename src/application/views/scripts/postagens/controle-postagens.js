...
Postagens.ControlePostagensWindowFilter = new Ext.ux.grid.GridFilters({
	local: false,
...
Postagens.ControlePostagensWindow = Ext.extend(Ext.grid.GridPanel, {
	id: 'ControlePostagensWindow',
	border: false,
	stripeRows: true,
	loadMask: true,
	sm: Postagens.sm,
	columnLines: true,
	plugins: [Postagens.ControlePostagensWindowFilter],
	listeners:{
		click:function(evento){
			var painel = Ext.getCmp('filterPanel_ControlePostagensWindow');
			if(painel){
				if(painel.isVisible()){
					painel.hide();
					Ext.getCmp('filterButton_ControlePostagensWindow').toggle(false);
				}
			}
		},
		beforerender:function(grid){
			Application.AccessController.applyPermission({
				defaultAction:'hide',
				items:[{
					objeto:'btnAdicionar_ControlePostagensWindow',
					acl:22,
					tipo:'componente'
				/*},{
	...
	initComponent: function(){
		...
		Ext.apply(this, {
			...
			tbar: [{
				...
				id:'filterButton_ControlePostagensWindow',
				...
				handler: function(botao, evento){
					...
					if(painel){
						...
						painel = new Ext.FormPanel({
							id:'filterPanel_ControlePostagensWindow',
							width:390,
							height:98,
							monitorValid:true,
							labelWidth:70,
							...
							bodyBorder: false,
							baseCls: 'floatfiltro',
							renderTo: Ext.getCmp('ControlePostagensWindow').getGridEl(),
							listeners: {
								'render': 
							...
							items:[{
								...
								items: [{
									...
									items:[{
										...
										xtype:'textfield',
										id:'filtro_ControlePostagensWindow',
										name:'filtro',
										listeners:{
											specialkey:function(owner,e){
												if (e.getKey() == 13){
													Ext.getCmp('btnexecfiltro_ControlePostagensWindow').execFiltro();
												}
											}
										}
									}]
									...
									items:[{
										...
										xtype: 'datefield',
										name: 'startdt',
										id: 'startdt_ControlePostagensWindow',
										vtype: 'daterange',
										format: 'd/m/Y',
										endDateField: 'enddt_ControlePostagensWindow',
										listeners:{
											specialkey:function(owner,e){
												if (e.getKey() == 13){
													Ext.getCmp('btnexecfiltro_ControlePostagensWindow').execFiltro();
									...
									items:[{
										...
										xtype: 'datefield',
										name: 'enddt',
										id: 'enddt_ControlePostagensWindow',
										format: 'd/m/Y',
										vtype: 'daterange',
										startDateField: 'startdt_ControlePostagensWindow',
										listeners:{
											specialkey:function(owner,e){
												if (e.getKey() == 13){
													Ext.getCmp('btnexecfiltro_ControlePostagensWindow').execFiltro();
							...
							buttons:[{
								...
								id:'btnexecfiltro_ControlePostagensWindow',
								iconCls: 'ux-gridfilter-text-icon',
								formBind:true,
								execFiltro:function(){
									var busca = Ext.getCmp('filtro_ControlePostagensWindow');
									var dtInicial = Ext.getCmp('startdt_ControlePostagensWindow');
									var dtFinal = Ext.getCmp('enddt_ControlePostagensWindow');
									var meBtn = Ext.getCmp('filterButton_ControlePostagensWindow');
									...
									Ext.getCmp('ControlePostagensWindow').store.on('beforeload', function(store, options) {
									...
									Ext.getCmp('filterPanel_ControlePostagensWindow').hide();
									Ext.getCmp('filterButton_ControlePostagensWindow').toggle();
	
									Ext.getCmp('ControlePostagensWindow').store.load();
								},
								handler: function(botao, evento){
									botao.execFiltro();
								}
				...
				handler: function(botao, evento){
					Ext.getCmp('filtro_ControlePostagensWindow').reset();
					Ext.getCmp('startdt_ControlePostagensWindow').reset();
					Ext.getCmp('enddt_ControlePostagensWindow').reset();
					Ext.getCmp('ControlePostagensWindow').store.reload();
					Postagens.ControlePostagensWindowFilter.clearFilters();
					var painel = Ext.getCmp('filterPanel_ControlePostagensWindow');
					if(painel){
						if(painel.isVisible()) painel.hide();
						meBtn = Ext.getCmp('filterButton_ControlePostagensWindow');
						meBtn.toggle(false);
						meBtn.setIconClass('icon-addfilter');
					}
				}
			...
			'->',
			{
				text: Application.app.language('controle.postagens.publicar'),
				iconCls: 'icon-ok',
				scope: this,
				handler: function(){
					this.publicaDespublica(true);
				}
			},{
				text: Application.app.language('controle.postagens.despublicar'),
				iconCls: 'silk-cross',
				scope: this,
				handler: function(){
					this.publicaDespublica(false);
				}
			},{
				text: Application.app.language('grid.form.add'),
				id:'btnAdicionar_ControlePostagensWindow',
				iconCls: 'silk-add',
				scope: this,
			...
			columns: [Postagens.sm, {
				...
				header: Application.app.language('controle.postagens.lida'),
				//width: 98,
				width: 70,
				align:'center',
				sortable: true,
				...
				dataIndex: 'fl_publicado',
				header: Application.app.language('controle.postagens.publicada'),
				width: 58,
				align:'center',
				sortable: true,
				renderer: function(data, cell, record, rowIndex, columnIndex, store) {
    				if(data === true){
						return '<img ext:qtitle="' + Application.app.language('controle.postagens.publicacao') + '" ext:qtip="' + Application.app.language('controle.postagens.publicada') + '" src="images/icons/tick.png" />';
					}
					else return '<img ext:qtitle="' + Application.app.language('controle.postagens.publicacao') + '" ext:qtip="' + Application.app.language('controle.postagens.nao.publicada') + '" src="images/icons/cross.png" />';
    			}
			},{
	...
	novoRegistro: function(){
		...
		document.getElementById('window_ControlePostagensForm').style.zIndex = 50;
	},
	
	publicaDespublica: function(publica){
		...
		if(publica) msg = Application.app.language('controle.postagens.publicar.confirm');
		else msg = Application.app.language('controle.postagens.despublicar.confirm');
		
		Application.app.confirm(Application.app.language('grid.form.confirm.title'), msg, function (opt) {
			if (opt === 'no') {
				return;
			}
			var id = [];
			...
			this.el.mask(Application.app.language('controle.postagens.aguarde'));
			Ext.Ajax.request({
				url: 'postagens/publish',
				params: {
					'id[]': id,
					publish: publica
				},
