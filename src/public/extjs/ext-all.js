...
(function () {
	...
	i.Observable.prototype = function () {
		var n = /^(?:scope|delay|buffer|single)$/,
		m = function (o) {
...
Ext.EventManager = function () {
	...
	b = document,
	u = window,
	g = "ie-deferred-loader",
	o = "DOMContentLoaded",
	e = {},
	h = /^(?:scope|delay|buffer|single|stopEvent|preventDefault|stopPropagation|normalized|args|delegate)$/;
...
Ext.apply(Ext.EventManager, function () {
	...
	h = 0,
	g = 0,
	d = Ext.isSafari ? Ext.num(navigator.userAgent.toLowerCase().match(/version\/(\d+\.\d)/)[1] || 2) >= 3.1 : !((Ext.isGecko && !Ext.isWindows) || Ext.isOpera);
	return {
...
(function () {
	...
	Ext.Element = function (n, o) {
		var p = typeof n == "string" ? i.getElementById(n) : n,
		q;
		if (!p) {
			return null
		}
	...
	var j = /\d+(px|em|%|en|ex|pt|in|cm|mm|pc)$/i,
	c;
	g.cache = {};
	g.dataCache = {};
	g.get = function (o) {
...
Ext.Element.addMethods(function () {
	...
	v = Ext.isIE ? "styleFloat" : "cssFloat",
	D = /alpha\(opacity=(.*)\)/i,
	l = /^\s+|\s+$/g,
	B = Ext.Element,
	d = "padding",
	...
	return {
		...
		getHeight: function (H) {
			var G = this,
			I = G.dom,
			F = s.max(I.offsetHeight, I.clientHeight) || 0;
		...
		getWidth: function (G) {
			var H = this,
			I = H.dom,
			F = s.max(I.offsetWidth, I.clientWidth) || 0;
			F = !G ? F : F - H.getBorderWidth("lr") - H.getPadding("lr");
			return F < 0 ? 0 : F
		},
		setWidth: function (G, F) {
			var H = this;
			G = H.adjustWidth(G); ! F || !H.anim ? H.dom.style.width = H.addUnits(G) : H.anim({
				width: {
					to: G
				}
			},
		...
		setHeight: function (F, G) {
			var H = this;
			F = H.adjustHeight(F); ! G || !H.anim ? H.dom.style.height = H.addUnits(F) : H.anim({
				height: {
					to: F
				}
			},
...
Ext.Element.addMethods(function () {
	...
	return {
		...
		getComputedHeight: function () {
			var c = this,
			b = Math.max(c.dom.offsetHeight, c.dom.clientHeight);
			if (!b) {
...
Ext.Element.addMethods(function () {
	...
	return {
		...
		anim: function (o, p, m, r, n, l) {
			...
			p = p || {};
			var q = this,
			s = Ext.lib.Anim[m](q.dom, o, (p.duration || r) || 0.35, (p.easing || n) || "easeOut", function () {
				if (l) {
					l.call(q)
				}
				if (p.callback) {
...
Ext.extend(Ext.util.MixedCollection, Ext.util.Observable, {
	...
	_sort: function (m, a, l) {
		var e, g, d = String(a).toUpperCase() == "DESC" ? -1 : 1,
		j = [],
		b = this.keys,
		h = this.items;
		l = l ||
...
Ext.layout.ColumnLayout = Ext.extend(Ext.layout.ContainerLayout, {
	...
	onLayout: function (d, j) {
		...
		var l = n.width - j.getPadding("lr") - this.scrollOffset,
		b = n.height - j.getPadding("tb"),
		m = l;
...
Ext.layout.VBoxLayout = Ext.extend(Ext.layout.BoxLayout, {
	...
	onLayout: function (e, E) {
		...
		var g = this.getItems(e),
		j,
		m,
		q,
		r = this.getTargetSize(E),
		k = r.width - E.getPadding("lr") - this.scrollOffset,
		y = r.height - E.getPadding("tb"),
		s = this.padding.left,
		n = this.padding.top,
		i = this.pack == "start",
		v = ["stretch", "stretchmax"].indexOf(this.align) == -1,
		B = k - (this.padding.left + this.padding.right),
		o = 0,
		A = 0,
		u = 0,
		C = 0,
		...
		var c = Math.max(0, y - this.padding.top - this.padding.bottom - C),
		D = c,
		z = [],
		x = [],
		p = 0,
		b = Math.max(0, k - this.padding.left - this.padding.right);
		Ext.each(g, function (h) {
			if (i && h.flex) {
...
Ext.layout.HBoxLayout = Ext.extend(Ext.layout.BoxLayout, {
	...
	onLayout: function (d, E) {
		...
		var e = this.getItems(d),
		j,
		c,
		q,
		r = this.getTargetSize(E),
		m = r.width - E.getPadding("lr") - this.scrollOffset,
		B = r.height - E.getPadding("tb"),
		u = this.padding.left,
		o = this.padding.top,
		i = this.pack == "start",
		z = ["stretch", "stretchmax"].indexOf(this.align) == -1,
		n = B - (this.padding.top + this.padding.bottom),
		C = 0,
		s = 0,
		v = 0,
		b = 0,
		...
		var x = Math.max(0, m - this.padding.left - this.padding.right - b),
		D = x,
		k = [],
		A = [],
		p = 0,
		a = Math.max(0, B - this.padding.top - this.padding.bottom);
		Ext.each(e, function (h) {
			if (i && h.flex) {
...
Ext.data.DataWriter.prototype = {
	...
	toHash: function (e) {
		var d = e.fields.map,
		c = {},
		b = (this.writeAllFields === false && e.phantom === false) ? e.getChanges() : e.data,
		a;
...
Ext.extend(Ext.data.JsonReader, Ext.data.DataReader, {
	...
	readRecords: function (a) {
		...
		var u = this.meta,
		m = this.recordType,
		h = m.prototype.fields,
		t = h.items,
		p = h.length,
		q;
		this.buildExtractors();
		var l = this.getRoot(a),
		k = l.length,
		j = k,
		r = true;
		if (u.totalProperty) {
...
Ext.data.ArrayReader = Ext.extend(Ext.data.JsonReader, {
	readRecords: function (q) {
		this.arrayData = q;
		var h = this.meta,
		d = h ? Ext.num(h.idIndex, h.id) : null,
		b = this.recordType,
		p = b.prototype.fields,
		y = [],
		e;
		if (!this.getRoot) {
...
Ext.data.DirectStore = function (a) {
	...
	Ext.data.DirectStore.superclass.constructor.call(this, Ext.apply(a, {
		proxy: (typeof(a.proxy) == "undefined") ? new Ext.data.DirectProxy(Ext.copyTo({},
		a, "paramOrder,paramsAsHash,directFn,api")) : a.proxy,
		reader: (typeof(a.reader) == "undefined" && typeof(a.fields) == "object") ? new Ext.data.JsonReader(Ext.copyTo({},
		a, "totalProperty,root,idProperty"), a.fields) : a.reader
	}))
};
...
Ext.ListView.ColumnResizer = Ext.extend(Ext.util.Observable, {
	...
	onEnd: function (l) {
		...
		g = o.columns,
		k = g.length;
		var q = this.view.innerHd.getWidth(),
		d = this.minPct * 100;
...
Ext.layout.ToolbarLayout = Ext.extend(Ext.layout.ContainerLayout, {
	...
	fitToSize: function (n) {
		...
		if (d > m || (this.hiddens && m >= b)) {
			var e, h = this.container.items.items,
			g = h.length,
			j;
			var a = 0;
			for (e = 0; e < g; e++) {
	...
	beforeMoreShow: function (b) {
		var g = this.container.items.items,
		a = g.length,
		k, j, d = function (h, c) {
