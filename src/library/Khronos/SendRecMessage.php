	...
	public function ExecSend() {
		try{
			...
			$messages = Doctrine_Query::create()
					...
					->where('m.sca_status_mensagem_id = 1 OR m.sca_status_mensagem_id = 3')
					->orderBy('m.data_mensagem ASC')
					->limit(20);
			$ultima_account = 0;
			foreach($messages->execute() as $message){
				
				if($ultima_account != $message->sca_account_id){
					$accountConfig = Doctrine_Query::create()				
						->from('ScaAccountRelationConfig c')
						->where('c.sca_account_id = ?', $message->sca_account_id);
					...
					foreach($accountConfig->execute() as $config){
						switch($config->sca_account_config_id){
							...
							case 13: 
								...
								break;
							case 17: 
								$SMTPSSL = $config->valor_parametro;
								break;
						}
					}
				...
				$ultima_account = $message->sca_account_id;
				...
				$mail->setBodyText(str_replace(chr(13) . chr(10) . chr(13) . chr(10), "", str_replace(chr(9), "", Khronos_SendRecMessage::remove_HTML($message->corpo))));					
